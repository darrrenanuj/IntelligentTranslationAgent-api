...
An intelligent translation middleware API that accepts a mapping file (typically authored by a Business Analyst), a Source JSON Schema, and a Target JSON Schema. It calls the Anthropic Claude API to generate translation logic between the two schemas, caches the resulting algorithm, and reuses it for all subsequent calls to transform source payloads into target payloads.

With this API, the development effort will be reduced by 80% as the entire translation logic will be generated by AI
and its mosttly the Business Analyst effort to get the mapping file documented and getting the source and target schema files as needed. This will be a great detail in the middleware world.
...
Spring boot application using the AWS SAM serverless architype compatible for AWS Lambda functions.
